...
class MalariaDemographics(Demographics):
    ...
    Uses :class:`~emodpy_malaria.demographics.malaria_node.MalariaNode`
    instead of plain :class:`~emod_api.demographics.node.Node` so that
    malaria-only demographics distributions (risk, innate immune) are
    available.

    Adds malaria-specific features:

    - :meth:`set_fertility_distribution` — age-and-year pregnancy rates
    - :meth:`set_risk_distribution` — heterogeneous biting risk via demographics
    - :meth:`set_innate_immune_distribution` — heterogeneous innate immunity
    - :meth:`add_vector_migration` — per-species vector migration
    - :meth:`add_weather` — weather files with node/idref validation

    Inherited from base (see :class:`~emod_api.demographics.demographics_base.DemographicsBase`):

    - :meth:`set_age_distribution` — initial age distribution (simple or complex)
    - :meth:`set_mortality_distribution` — gendered mortality (complex)
    - :meth:`set_susceptibility_distribution` — susceptibility (simple or complex)
    - :meth:`set_prevalence_distribution` — initial prevalence (simple)
    - :meth:`set_migration_heterogeneity_distribution` — migration heterogeneity (simple)
    """
    ...
    def add_vector_migration(
        ...
        *,
        ...
        Provide either ``data`` (a :class:`~emodpy_malaria.migration.VectorMigrationData`
        ...
        Args:
            data: VectorMigrationData object, or None if using vector_migration_filename_path.
            species: Species **Name** to target.
            vector_migration_filename_path: Path to a pre-existing binary migration file.
            ...
            vector_migration_modifier_equation: Functional form of vector migration modifiers.
                ...
                :attr:`ModifierEquationType.LINEAR`.
            vector_migration_habitat_modifier: Preference of a vector to migrate toward a node
                with more habitat. This applies only to female migrating vectors.
            vector_migration_food_modifier: Preference of a vector to migrate toward a node
            ...
            vector_migration_stay_put_modifier: Preference of a vector to remain in its current
                node rather than migrate to another node. This applies only to female migrating
                vectors.
            filename: Output path for the binary file when using ``data``.
                If None, auto-generates as ``vector_migration_{species}.bin``.
            user_notes: Free-text description of the migration data, stored in the metadata
    ...
    def add_weather(
        ...
        *prefix* is prepended to each file name
        ...
        Args:
            data: :class:`~emodpy_malaria.weather.WeatherSet` containing
                air temperature, rainfall, and relative humidity.
            prefix: String prepended to each weather file name. Default
                ``""`` (no prefix).
            update_resolution: Climate update frequency. Default
    ...
    def from_template_node(cls,
        ...
        """Create a single-node MalariaDemographics using a :class:`MalariaNode`.

        Args:
            lat: Latitude of the node. Default 0.
            lon: Longitude of the node. Default 0.
            pop: Initial population. Default 1,000,000.
            name: Node name. Default ``"Erewhon"``.
            forced_id: Node ID. Default 1.
    ...
    def from_file(cls, path: str) -> "MalariaDemographics":
        ...
        Uses :class:`MalariaNode` so that malaria-specific distributions
        (risk, innate immune) are preserved when loading from file.

        Args:
            path: Path to an EMOD demographics JSON file.
    ...
    def set_fertility_distribution(self,
        ...
        Args:
            distribution: A
                :class:`~emod_api.demographics.fertility_distribution.FertilityDistribution`
                defining age-and-year-specific pregnancy rates.
            node_ids: Node id(s) to apply to. ``None`` targets the default node.
    ...
    def set_risk_distribution(self, distribution: BaseDistribution, node_ids: list[int] = None) -> None:
        ...
        Args:
            distribution: A
                :class:`~emodpy_malaria.utils.distributions.BaseDistribution`
    ...
    def set_innate_immune_distribution(
        ...
        *innate_immune_variation_type*.
        ...
        Args:
            ...
            innate_immune_variation_type: How the drawn value modifies innate
                immunity. Accepts a :class:`~emodpy_malaria.utils.emod_enum.InnateImmuneVariationType`
