...
def add_report_vector_genetics(task, manifest,
    ...
    Args:
        ...
        start_day (int): the day of the simulation to start reporting data
        end_day (int): the day of the simulation to stop reporting data
        node_ids (list): the list of nodes in which to collect data, empty or None means all nodes
        species (str): the species to include information on
        gender (str): gender of species to include information on. Default: "VECTOR_FEMALE",
            other options: "VECTOR_MALE", "VECTOR_BOTH_GENDERS"
        include_vector_state (bool): if True(1), adds the columns for vectors in the different states (i.e Eggs, Larva, etc)
        include_death_state (bool): if True(1), adds columns for the number of vectors that died in this state during this
        ...
        stratify_by (str): the way to stratify data. Default: "GENOME",
            other options: "SPECIFIC_GENOME", "ALLELE", "ALLELE_FREQ"
        combine_similar_genomes (bool): if True(1), genomes are combined if for each locus (ignoring gender) the set of allele
            of the two genomes are the same (i.e. 1-0 is similar to 0-1). Depends on: "GENOME", "SPECIFIC_GENOME"
            specific_genome_combinations_for_stratification (list): if stratifying by "SPECIFIC_GENOME", then use these genomes
            to stratify by.<br>
            Example:
        ...
        allele_combinations_for_stratification (list): if stratifying by "ALLELE", then also add these allele name
            combos to the stratification.<br>
            Example:

                [[ "a0", "b0" ], [ "a1", "b1" ]]

        alleles_for_stratification (list): For example:

            [ "a0", "a1", "b0", "b1" ]

        filename_suffix (str): augments the filename of the report. If multiple reports are being generated,
...
def add_report_vector_stats(task, manifest,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter, if left as None, reporter is returned (used for unittests)
        manifest (ModuleType): schema path file
        species_list (list): a list of species to include information on, default of None or [] means "all species"
        stratify_by_species (bool): if True(1), data will break out each the species for each node
        include_death_state (bool): if True(1), adds columns for the number of vectors that died in this state during this
            time step as well as the average age.  It adds two columns for each of the following states: ADULT,
            INFECTED, INFECTIOUS, and MALE
        include_wolbachia (bool): if True(1), add a column for each type of Wolbachia
        include_gestation (bool): if True(1), adds columns for feeding and gestation
        include_microsporidia (bool): if True(1), adds columns for the number of vectors that have microsporidia in
...
def add_malaria_summary_report(task, manifest,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter, if left as None, reporter is returned (used for unittests)
        manifest (ModuleType): schema path file
        start_day (int): the day of the simulation to starts collecting data for the report
        end_day (int): the day of the simulation to stop reporting data
        node_ids (list): a list of nodes from which to collect data for the report
        reporting_interval (float): Defines the cadence of the report by specifying how many time steps to collect data
            before writing to the file
        must_have_ip_key_value (str): a "Key:Value" pair that the individual must have in order to be included. Empty string
            means don't look at IPs (individual properties)
        must_have_intervention (str): the name of the an intervention that the person must have in order to be included.
            Empty string means don't look at the interventions
        include_time_age_pfpr_bins (bool): When set to true, the 'DataByTimeAndPfPRBinsAndAgeBins' element is included
            in the report.  Default is true.  You can save disk space by setting this to false.
        include_time_age_pfpr_infectious_bins (bool): When set to true, the 'DataByTimeAndInfectiousnessBinsAndPfPRBinsAndAgeBins'
            element is included in the report.  Default is true.  You can save disk space by setting this to false.
        add_true_density (bool): If set to true, four new channels will be added to the report that use true density instead
            of measured.<br>
            These additional channels are 'PfPR_2to10-True', 'PfPR by Age Bin-True', 'Pf Gametocyte Prevalence by Age Bin-True', and
            'Mean Log Parasite Density by Age Bin-True'.<br>
            The true densities will be compared to thresholds 'parasite_detection_threshold' and 'gametocyte_detection_threshold'. The Default is False.
        parasite_detection_threshold (float): Used when 'add_true_density' is true.  The true parasite density
            is compared against this threshold.<br>
            It impacts the 'PfPR_2to10-True', 'PfPR by Age Bin-True', and 'Mean Log Parasite Density by Age Bin-True' channels.  Default is zero.
        gametocyte_detection_threshold (float): Used when 'add_true_density' is true.  The true gametocyte
            density is compared against this threshold.  It impacts the 'Pf Gametocyte Prevalence by Age Bin-True' channel.
            Default is zero.
        add_hrp2_prevalence (bool): If true, the 'PfPR_2to10-HRP2' and the 'PfPR by Age Bin-HRP2' channels will be added.
            These channels will use 'Detection_Threshold_True_HRP2' to determine if person's HRP2 level counts towards prevalence.
        hrp2_detection_threshold (float): Used when 'add_hrp2_prevalence' is true.  If the true HRP2 value is greater than this threshold,
            the prevalence will be increased in the 'PfPR_2to10-HRP2' and the 'PfPR by Age Bin-HRP2' channels.
        age_bins (list): The max age in years per bin, listed in ascending order. Use a large value for the last bin,
            to collect all remaining individuals
        infectiousness_bins (list): The bins for aggregating infectiousness data in the report
        max_number_reports (int): the maximum number of report output files that will be produced for a given simulation
        parasitemia_bins (list): Parasitemia bins on which to aggregate. A value <= 0 in the first bin indicates that
            uninfected individuals are added to this bin. You must sort your input data from low to high.
        pretty_format (bool): if True(1) sets pretty JSON formatting, which includes carriage returns, line feeds, and spaces
...
def add_malaria_patient_json_report(task, manifest,
    ...
    Args:
        ...
        end_day (int): the day of the simulation to stop reporting data
        node_ids (list): a list of nodes from which to collect data for the report
        min_age_years (float): minimum age in years of people to collect data on
        max_age_years (float): maximum age in years of people to collect data on
        must_have_ip_key_value (str): a "Key:Value" pair that the individual must have in order to be included. Empty string
            means don't look at IPs (individual properties)
        must_have_intervention (str): the name of the intervention that the person must have in order to be included.
            Empty string means don't look at the interventions
        filename_suffix (str): augments the filename of the report. If multiple reports are being generated,
            this allows you to distinguish among the multiple reports
...
def add_malaria_cotransmission_report(task, manifest,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter, if left as None, reporter is returned (used for unittests)
        manifest (ModuleType): schema path file
        start_day (int): the day to start collecting data for the report.
        end_day (int): the day of the simulation to stop reporting data
        node_ids (list): list of nodes for which to collect data for the report
        min_age_years (float): minimum age in years of people to collect data on
        max_age_years (float): maximum age in years of people to collect data on
        include_human_to_vector (int): if set to 1, Human-to-Vector transmission events will be included.  One can identify
...
def add_report_malaria_filtered(task, manifest,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter, if left as None, reporter is returned (used for unittests)
        manifest (ModuleType): schema path file
        start_day (int): the day of the simulation to start collecting data
        end_day (int): the day of simulation to stop collecting data
        node_ids (list): list of nodes for which to collect the data, None or [] collects all the nodes
        min_age_years (float): Minimum age in years of people to collect data on
        max_age_years (float): Maximum age in years of people to collect data on
        must_have_ip_key_value (str): a "Key:Value" pair that the individual must have in order to be included. Empty string
            means don't look at IPs (individual properties)
        must_have_intervention (str): the name of the an intervention that the person must have in order to be included.
            Empty string means don't look at the interventions
        has_interventions (list): a list of intervention names, a channel is added to the report for each InterventionName
        ...
        include_30day_avg_infection_duration (bool): if True(1) the '30-Day Avg Infection Duration' channel is included
            in the report
        filename_suffix (str): augments the filename of the report. If multiple reports are being generated,
            this allows you to distinguish among the multiple reports
...
def add_spatial_report_malaria_filtered(task, manifest,
    ...
    Args:
        ...
        start_day (int): the day of the simulation to start collecting data
        end_day (int): the day of simulation to stop collecting data
        reporting_interval (int): defines the cadence of the report by specifying how many time steps to collect data before
            writing to the file.
        node_ids (list): list of nodes for which to collect the data, None or [] collects all the nodes
        min_age_years (float): Minimum age in years of people to collect data on
        max_age_years (float): Maximum age in years of people to collect data on
        ...
        must_have_intervention (str): the name of the an intervention that the person must have in order to be included.
            Empty string means don't look at the interventions
        spatial_output_channels (list): list of names of channels you want to have output for. Available channels are:
            "Adult_Vectors", "Air_Temperature", "Births", "Blood_Smear_Gametocyte_Prevalence",<br>
            "Blood_Smear_Parasite_Prevalence", "Campaign_Cost", "Daily_Bites_Per_Human", "Daily_EIR", "Disease_Deaths",<br>
            "Fever_Prevalence", "Human_Infectious_Reservoir", "Infected", "Infectious_Vectors", "Land_Temperature",<br>
            "Mean_Parasitemia", "New_Clinical_Cases", "New_Infections", "New_Reported_Infections", "New_Severe_Cases",<br>
            "PCR_Gametocyte_Prevalence", "PCR_Parasite_Prevalence", "PfHRP2_Prevalence", "Population", "Prevalence",<br>
            "Rainfall", "Relative_Humidity", "True_Prevalence"<br>
...
def add_report_event_counter(task, manifest,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter, if left as None, reporter is returned (used for unittests)
        manifest (ModuleType): schema path file
        start_day (int): the day of the simulation to start counting events
        end_day (int): the day of simulation to stop collecting data
        node_ids (list): list of nodes in which to count the events
        event_trigger_list (list): list of events which to count
        min_age_years (float): Minimum age in years of people to collect data on
        max_age_years (float): Maximum age in years of people to collect data on
        must_have_ip_key_value (str): a "Key:Value" pair that the individual must have in order to be included. Empty string
            means don't look at IPs (individual properties)
...
def add_sql_report_malaria(task, manifest,
    ...
    Args:
        ...
        manifest (ModuleType): schema path file
        start_day (int): the day of the simulation to start collecting data
        end_day (int): the day of the simulation to stop collecting data
        include_infection_table (bool): if True(1), include the table that provides data at each time step for each active
            infection
        include_health_table (bool): if True(1), include the table that provides data at each time step for a person's health
        include_drug_table (bool): if True(1), include the table that provides data at each time step for each drug used
        include_individual_properties (bool): if True(1), add columns to the Health table for each Property(key).
...
def add_malaria_immunity_report(task, manifest,
    ...
    Args:
        ...
        start_day (int): the day of the simulation to start collecting data
        end_day (int): the day of simulation to stop collecting data
        node_ids (list): list of nodes for which to collect data
        reporting_interval (int): defines the cadence of the report by specifying how many time steps to collect data before
            writing to the file.
        max_number_reports (int): the maximum number of report output files that will be produced for a given simulation
        age_bins (list): The max age in years per bin, listed in ascending order. Use a large value for the last bin,
...
def add_malaria_survey_analyzer(task, manifest,
    ...
    Args:
        ...
        reporting_interval (float): defines the cadence of the report by specifying how many time steps to collect data
            before writing to the file
        event_trigger_list (list): list of individual events to include into the report
        max_number_reports (int): the maximum number of report output files that will be produced for a given simulation
        node_ids (list): list of nodes for which to collect data
        ip_key_to_collect (str): name of the Individual Property Key whose value to collect.  Empty string means
            collect values for all Individual Properties
        must_have_ip_key_value (str): a "Key:Value" pair that the individual must have in order to be included. Empty string
            means don't look at IPs (individual properties)
        must_have_intervention (str): the name of the an intervention that the person must have in order to be included.
            Empty string means don't look at the interventions
        pretty_format (int): if True(1), sets pretty JSON formatting, which includes carriage returns, line feeds, and spaces
            for easier readability. The default, 0 (false), saves space where everything is on one line.
        filename_suffix (str): augments the filename of the report. If multiple reports are being generated,
            this allows you to distinguish among the multiple reports
...
def add_report_infection_stats_malaria(task, manifest,
    ...
    Args:
        ...
        start_day (int): the day of the simulation to start collecting data
        end_day (int): the day of the simulation to stop collecting data
        reporting_interval (int): defines the cadence of the report by specifying how many time steps to collect data
            before writing to the file
        include_hepatocyte (bool): If set to True, then a column will be added to the report providing the count of the
            infected hepatocytes. Default is True.
        hepatocyte_threshold (int): If the column is included and the number of hepatocytes related to this
        ...
        include_irbc (bool): If set to True, then a column will be added to the report with the number of Infected Red
            Blood Cells from this infection. Default is True.
        irbc_threshold (int): If the column is included and the number of IRBCs related to this infection are greater
        ...
        include_gametocyte (bool): If set to True, then a column will be added to the report that contains the number
            of gametocytes (male & female) from this infection. Default is True.
        gametocyte_threshold (int): If the column is included and the number of gametocytes related to this infection
...
def add_report_node_demographics(task, manifest,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter, if left as None, reporter is returned (used for unittests)
        manifest (ModuleType): schema path file
        age_bins (list): the age bins (in years) to aggregate within and report. An empty array does not stratify by age. You
            must sort your input data from low to high.
        ip_key_to_collect (str): The name of the Individual Properties Key by which to stratify the report.
            An empty string does not stratify by Individual Properties
        stratify_by_gender (bool): if True(1), to stratify by gender. Set to False (0) to not stratify by gender.
...
def add_report_node_demographics_malaria(task, manifest,
    ...
    Args:
        ...
        age_bins (list): the age bins (in years) to aggregate within and report. An empty array does not stratify by age. You
            must sort your input data from low to high.
        ip_key_to_collect (str): The name of theIndividualProperties key by which to stratify the report.
            An empty string does not stratify by Individual Properties
        stratify_by_gender (bool): if True(1), to stratify by gender. Set to False (0) to not stratify by gender.
        stratify_by_clinical_symptoms (bool): if set to True(1), the data will have an extra stratification for people who have
...
def add_report_node_demographics_malaria_genetics(task, manifest,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter, if left as None, reporter is returned (used for unittests)
        manifest (ModuleType): schema path file
        barcodes (list): a list of barcode strings. The report contains the number of human infections with each barcode.
        ...
        drug_resistant_strings (list): a list of strings representing the set of drug resistant markers.  A column will be
        ...
        hrp_strings (list): A list of strings representing the set of HRP markers.  A column will be created with the
        ...
        drug_resistant_and_hrp_statistic_type (DrugResistantAndHRPStatisticType): indicates the statistic in the Drug Resistant & HRP columns:
        ...
        ip_key_to_collect (str): The name of theIndividualProperties key by which to stratify the report.
            An empty string does not stratify by Individual Properties
        stratify_by_gender (bool): if True(1), to stratify by gender. Set to False(0) to not stratify by gender.
        include_identity_by_xxx (bool): if True(1), include columns about the average Identity By State (IBS) and
...
def add_report_vector_migration(task, manifest,
    ...
    Args:
        ...
        start_day (int): the day of the simulation to start collecting data
        end_day (int): the day of the simulation to stop collecting data
        species_list (list[str]): a list of species to include information on, default of None or [] means "all species".
        must_be_in_state (list[VectorState]): A list of vector states for which you want to record the migration. Only STATE_MALE,
            STATE_ADULT, STATE_INFECTED, STATE_INFECTIOUS migrate.
        must_be_from_node (list[int]): A list of node IDs. A vector must be traveling FROM one of these nodes to be recorded into
            the report. Empty list means vectors traveling from any/all nodes will be recorded.
        must_be_to_node (list[int]): A list of node IDs. A vector must be traveling TO one of these nodes to be recorded into the
            report. Empty list means vectors traveling to any/all nodes will be recorded.
        include_genome_data (bool): If set to True, adds a Genome column for migrating vectors. Vectors with no
            custom alleles will still have their sex alleles listed.
        filename_suffix (str): Augments the filename of the report. If multiple reports are being generated, this allows you
...
def add_event_recorder(task, event_list: list = None,
    ...
    """
    Adds **ReportEventRecorder** report to the simulation.<br>
    The [health events and interventions report](https://emod.idmod.org/emodpy-malaria/emod/software-report-event-recorder/)
    (ReportEventRecorder.csv) provides information on each individual’s demographics and health status at the time of an event.
    Additionally, it is possible to see the value of specific IndividualProperties, as assigned in the demographics file
    (see [NodeProperties and IndividualProperties](https://emod.idmod.org/emodpy-malaria/emod/model-properties/)
    for more information).

    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter
        event_list (list): a list of events to record or exclude, depending on value of only_include_events_in_list
        only_include_events_in_list (bool): if True, only record events listed.  if False, record ALL events EXCEPT for the
            ones listed
        ips_to_record (list): list of individual properties to include in report
        start_day (int): The day of the simulation to start collecting data
        end_day (int): The day of the simulation to stop collecting data.
        node_ids (list): Data will be collected for the nodes in this list, if None - all nodes have data collected.
        min_age_years (float): Minimum age in years of people to collect data on
        max_age_years (float): Maximum age in years of people to collect data on
        must_have_ip_key_value (str): A Key:Value pair that the individual must have in order to be included. Empty string
        ...
        must_have_intervention (str): The name of the intervention that the person must have in order to be included.
            Empty string means don't look at the interventions
        property_change_ip_to_record (str): If the string is not empty, then the recorder will add the PropertyChange event to
    ...
    """
    # Documentation: https://emod.idmod.org/emodpy-malaria/emod/software-report-event-recorder/
    if not event_list:
...
def add_node_event_recorder(task, event_list: list = None,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter
        event_list (list): A list of node-level events to record or exclude, depending on value of
            only_include_node_events_in_list
        only_include_events_in_list (bool): If True, only record node-level events listed.  if False, record ALL
            node-level events EXCEPT for the ones listed
        stats_by_node_properties (list): Specifies an array of (optional) node property keys, as defined in
        ...
        stats_by_individual_properties (str): Specifies an array of (optional) individual property keys, as defined in
...
def add_coordinator_event_recorder(task, event_list: list = None,
    ...
    Args:
        task (emodpy.emod_task.EMODTask): task to which to add the reporter
        event_list (list): A list of coordinator-level events to record or exclude, depending on value of
...
def add_report_fpg_output(task, manifest,
    ...
    Args:
        ...
        must_have_intervention (str): the name of the an intervention that the person must have in order to be included.
            Empty string means don't look at the interventions
        include_genome_ids (bool): Add a column that has a list of Genome IDs (hashcode) for the person.
        minimum_parasite_density (float): The minimum density that the infection must have to be included in the list
            of infections. A value of zero implies include all infections. Number of asexual parasites
            per micro liter of blood.
        sampling_period (float): The number of days between sampling the population. This implies one should get data
...
def add_report_fpg_new_infections(task, manifest,
    ...
    Args:
        ...
        must_have_intervention (str): the name of the intervention that the person must have in order to be included.
            Empty string means don't look at the interventions
        filename_suffix (str): Augments the filename of the report. If multiple reports of this class are being generated,
            this allows you to distinguish among them.
        report_crossover_data_instead (bool): If True, instead of reporting new infections in detail, the report will contain
...
def add_report_antibodies(task, manifest,
    ...
    Args:
        ...
        filename_suffix (str): Augments the filename of the report. If multiple reports of this class are being generated,
            this allows you to distinguish among them.
        reporting_interval (int): interval (in days) at which to report antibody data.
        contain_capacity_data (bool): If true, the data for each antibody is the capacity of the antibody,
        ...
        infected_only (bool): If true, only individuals who are currently infected will have their antibody data recorded.
...
class ReportAntibodies(BuiltInReporter):
    ...
    def config(self, config_builder, manifest):
        self.class_name = "ReportAntibodies"
        report_params = s2c.get_class_with_defaults("ReportAntibodies", manifest.schema_file)
        report_params = config_builder(report_params)
        report_params.finalize()
        report_params.pop("Sim_Types")
        self.parameters.update(dict(report_params))
