...
def set_param_fn(config):
    ...
    Returns:
        completed config
    """
    from emodpy_malaria.vector_config import ModifierEquationType
    ...
    vector_config.add_vector_migration(config, species="gambiae",
                                       vector_migration_filename="Local_Vector_Migration_3_Nodes_One_Way.bin",
                                       x_vector_migration=1,
                                       vector_migration_modifier_equation=ModifierEquationType.EXPONENTIAL,
                                       vector_migration_habitat_modifier=0, vector_migration_food_modifier=0,
...
def build_demog():
    ...
    demog = Demographics.from_file("Assets/demographics_3_nodes.json")
...
def general_sim():
    ...
    experiment_name = "Vector migration and surveillance example"
    ...
    EMODTask.add_py_path(task, manifest.path_to_python_vm)

    # adding the migration files to the assets
    ...
    add_report_vector_migration(task, manifest,
                                ...
                                end_day=26,
                                species_list=None,
                                must_be_in_state=[VectorState.STATE_MALE, VectorState.STATE_ADULT],
                                must_be_from_node=None,
                                must_be_to_node=None,
                                include_genome_data=True,
                                filename_suffix="vector_migration_example")
