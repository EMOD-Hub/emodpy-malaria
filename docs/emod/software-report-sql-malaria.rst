...
The SqlReportMalaria report extends :doc:`software-report-sql` with malaria-specific health and
infection data. It is only available when **Simulation_Type** is MALARIA_SIM. The output is a
multi-table SQLite relational database (see `DB Browser for SQLite <https://sqlitebrowser.org/>`_
...
See :doc:`software-report-sql-malaria-genetics`.
...
The report produces all tables from :doc:`software-report-sql`. The Health and InfectionData
